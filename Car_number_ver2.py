...
for i in range(min_width, max_width + 1):
    ...
    for i, plate_img in enumerate(plate_imgs):
        ...
        if len(result_chars) < 7:
            continue
        if not has_korean(result_chars):
            print("인식한 문자에 한글이 포함되어 있지 않습니다.")
            continue
        korean_count = count_korean(result_chars)
        if korean_count >= 2:
            continue
        print(f"{result_chars} read_lan")
        ...
        # cv2.imshow(f"Result Image {i + 1}", img_result)
